...
int WINAPI otd_wcslen(wchar_t * str)
{
    int ret = 0;
    LPWORD ptr = (LPWORD)str;
    while(str && (*ptr)!=0)
    {
         ++ret;
         ++ptr;
...
int  WINAPI otd_text_set    (lpotd_block bl,wchar_t * str)
{
 if(bl && bl->block_type == OTD_BLOCKTYPE_TEXT)
 {
  ...
  WORD  capasity  = (WORD)OTD_TEXT_GET_CAPASITY(bl);
  ltxt->len       = KeRTL::MIN((WORD)(1+wcslen(str)),capasity);
  memcpy(ltxt->text,str,sizeof(wchar_t)*ltxt->len);
  return (int)ltxt->len;
 }
 return 0;
}
