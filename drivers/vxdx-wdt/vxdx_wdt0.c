/*
 *	VortexDX internal Watchdog Timer 0 driver
 *
 *      Release 0.20
 *
...
#define WATCHDOG_NAME "VortexDX WDT0"
#define PFX WATCHDOG_NAME ": "
#define WATCHDOG_TIMEOUT 60		/* 60 sec default timeout */
#define WATCHDOG_EARLY_DISABLE 1	/* Disable until userland kicks in */

/* Registers for access to the index registers */
#define WDT_INDX_REG 0x22
#define WDT_DATA_REG 0x23
#define WDT_REG_LEN  2

/* Index registers */
#define WDT_ENDIS_IDX 0x37
#define WDT_CFG_IDX   0x38
#define WDT_COUNT_IDX 0x39
#define WDT_RESET_IDX 0x3c

/* Start WDT0 I/O address */
const int wdt_io = WDT_INDX_REG;
...
static inline uint8_t read_idx_reg(uint8_t index)
{
	outb(index, WDT_INDX_REG);
	return inb(WDT_DATA_REG);
}

static inline void write_idx_reg(uint8_t index, uint8_t value)
{
	outb(index, WDT_INDX_REG);
	outb(value, WDT_DATA_REG);
}

static void vxdx_init(void)
{
	spin_lock(&io_lock);
	/* Set the system reset as watchdog signal (p.92) */
	write_idx_reg(WDT_CFG_IDX, 0xD0);
	spin_unlock(&io_lock);
}
...
static void wdt_ping(void)
{
	spin_lock(&io_lock);
	/* Set the 6 bit */
	write_idx_reg(WDT_RESET_IDX, read_idx_reg(WDT_RESET_IDX) | 0x40);
	spin_unlock(&io_lock);
}
...
static void wdt_enable(void)
{
	spin_lock(&io_lock);
	/* Set the 6 bit */
	write_idx_reg(WDT_ENDIS_IDX, read_idx_reg(WDT_RESET_IDX) | 0x40);
	spin_unlock(&io_lock);
}
...
static uint8_t wdt_running(void)
{
	...
	spin_lock(&io_lock);
	/* Read the 6 bit */
	t = read_idx_reg(WDT_ENDIS_IDX) >> 6;
	spin_unlock(&io_lock);
...
static int wdt_set_heartbeat(int t)
{
	...
	write_idx_reg(WDT_COUNT_IDX, val & 0xFF);
	write_idx_reg(WDT_COUNT_IDX + 1, (val >> 8) & 0xFF);
	write_idx_reg(WDT_COUNT_IDX + 2, (val >> 16) & 0xFF);
	/* Enable the WDT */
	write_idx_reg(WDT_ENDIS_IDX, read_idx_reg(WDT_RESET_IDX) | 0x40);
	spin_unlock(&io_lock);
...
static int __init wdt_init(void)
{
	...
	printk(KERN_INFO PFX "WDT0 driver for VortexDX initializing\n");
...
MODULE_DESCRIPTION("VortexDX internal WDT0 driver");
