...
#define DELAY_ON_CHANNEL ndelay(1500)
...
void __aic124dev_start_scan(lpaic124dev dev,int no_real_start)
{
    ...
    if(dev->channels_count)
    {
        ...
        if(dev->aib_use)
        {
            ...
            //outb  (dev->aib_current->channels[0],dev->base_port+AIC124_WR_REGB_DISCRETE_OUT);
            //wmb();
        }
        outb_p(dev->channel_numbers[0],dev->base_port+AIC124_W_REGB_ADC_CHANNEL);
        //wmb();
        //outb  (dev->channel_numbers[0],dev->base_port+AIC124_W_REGB_ADC_CHANNEL);
...
void aic124dev_timer  (unsigned long arg)
{
    ...
    if(dev && sizeof(*dev) == dev->isize )
    {
        if(0!=atomic_read(&dev->active))
        {
            ...
            if(0 == atomic_read(&dev->scan_state)) //if scanning process is inactive - start him
            {
...
void __always_inline __aic124dev_next_input(lpaic124dev dev)
{
    ...
    if(!dev->scan_start_time.tv_sec)
       do_gettimeofday(&dev->scan_start_time);
    dev->curr_data_buff->tmstamp = dev->scan_start_time;
...
irqreturn_t __aic124dev_irq_handler(int irq_num, void * arg)
{
    ...
    lpaic124dev dev = (lpaic124dev) arg;
    if(dev && dev->isize == sizeof(*dev) && dev->irq_num == irq_num)
    {
        rmb();
...
//              value = dev->channel_numbers[dev->channel_idx];
...
#endif
            switch(atomic_read(&dev->scan_state))
            {
            case __AIC124_DEV_SCAN_TEST_SIGNAL_1:
#ifndef _REAL_DEVICE
                value = 0x7D0;
            ...
            if(0!=atomic_read(&dev->active) && __AIC124_DEV_SCAN_NONE < __aic124dev_prepare_next_scan(dev))
            {
              #ifdef _REAL_DEVICE
                DELAY_ON_CHANNEL;
                
                outb_p(dev->regb0wr,dev->base_port);//Start next scan
              #else
               tasklet_schedule(&dev->irq_emul_tasklet); 
              #endif
